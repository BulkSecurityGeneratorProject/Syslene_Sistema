...
export class LocalidadeComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.localidadeService
      ...
      .subscribe(
        (res: HttpResponse<ILocalidade[]>) => this.paginateLocalidades(res.body, res.headers),
  ...
  registerChangeInLocalidades() {
    this.eventSubscriber = this.eventManager.subscribe('localidadeListModification', response => this.loadAll());
  }
